...
const MAX_ENERGY = 1250
const MAX_OFFSPRINGS = 4

class Plant{
    constructor(pos, ranges){
        this.gen = {
            long_sec: random(5, 12),
            prob_repro: random(0.0003, 0.0005),
            precision_light: random(0.4, 0.8),     //1 is max precision, 0 lowest random(0.2, 05)
            angle_mult: random(0.5, 0.8),         //1 is max aperture (best) random(0.3, 0.5),      
            growth_rate: random(0.01, 0.02)
        }
    ...
    getNewAngle(oldAngle, pos){
        ...
        let maxError = radians(180) * (1 - this.gen.precision_light);
    ...
    getEnergy(pos){
        let d = dist(pos.x, pos.y, sun.x, sun.y)
    ...
    update(){
        ...
        this.energy -= (this.gen.growth_rate * 0.25) + 0.075
