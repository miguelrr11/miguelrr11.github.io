class Input {
    ...
    execute() {
        if(this.func) {
            let sentence = this.sentence.replace(/π/g, " ")
            if(this.arg) this.func(sentence)
            else this.func()
            // this.sentence = ""
            // this.clippedSentence = ""
            // this.cursorPos = 0
    ...
    setClippedSentence() {
        ...
        let margin = bordeMIGUI + text_offset_xMIGUI
        let maxTextWidth = this.w - 2 * margin
        let sentenceToClip = this.sentence.replace(/π/g, " ")
        this.clippedSentence = getClippedTextByWidth(sentenceToClip, this.firstCursor, maxTextWidth)
    }

    write(c) {
        // very ugly fix for p5js 2.0 setting 0 width to spaces
        let charToInsert = c == " " ? "π" : c
        this.sentence = this.insertCharAt(this.sentence, charToInsert, this.cursorPos)
        this.cursorPos++
    ...
    show() {
        if(this.errorFrames > 0){
            this.errorFrames--
            let mul = mapp(this.errorFrames, 6 * 3, 0, 5, 1)
            let dx = Math.cos(this.errorFrames) * mul
            this.pos.x = this.initialPosX + dx
        }
        ...
        //replace every "π" with " "
        let finalSentence = this.clippedSentence.replace(/π/g, " ")
        ...
        if(this.active) {
            ...
            if(this.firstCursor + this.relCursorPos === 0) x -= 3
