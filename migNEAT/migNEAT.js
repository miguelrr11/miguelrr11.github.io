...
class NN{
    ...
    initNN(n_in, n_out){
        ...
        for(let i = 0; i < n_out; i++){ 
            let neuron = new Neuron(this.func_out, this.idCounter, getBias(), false, true)
        ...
        for (let inp of this.inputs) {
            for (let out of this.outputs) {
                if (Math.random() < 0.15) {
                    let conn = new Connection(inp.id, out.id, getGaussianWeight())
        ...
        if(!connectionMade){
            ...
            let conn = new Connection(randomInput.id, randomOutput.id, getGaussianWeight())
            conn.innovationNumber = this.population.getInnovationNumber(conn)
            this.connections.push(conn);
        }
    }
    ...
    createRandomHidden(){
    	let hidden = new Neuron(this.func_hidden, this.idCounter++, getBias())
	...
	createRandomConnection() {
        ...
        let conn = new Connection(newConnection.from, newConnection.to, getGaussianWeight())
        conn.innovationNumber = this.population.getInnovationNumber(conn)
	    this.connections.push(conn);
    ...
    show(){
	    ...
	    for(let p of this.particles){ 
	    	...
	    	p.show(round(neuron.value, 1), round(neuron.bias, 1), out)
	    }
        noFill()
        stroke(150)
        strokeWeight(1)
...
class Population{
    ...
    evolvePopulation(){
        ...
        //seleccion NNs
        let elites = this.selectNNs()
        //console.log("elites " + elites.length)
        let crossNNs = []
        //console.log(elites)

        //crossover
        while(crossNNs.length < this.size){
            let p1Index = Math.floor(Math.random() * elites.length);
            let p1 = elites[p1Index]
    ...
    */
    ...
    calculateDistance(nn1, nn2) {
        ...
        const C3 = 0.4; // Coefficient for weight differences
    ...
    selectNNs() {
        let res = [];
        let superElite = undefined;

        for (let [idx, species] of this.species.entries()) {
            // Normalization
            let maxFit = 0;
            for (let nn of species.NNs) {
                if (nn.fitness === 4) superElite = nn;
            ...
            // Remove the lower half, unless there's only one in NNs
            if (species.NNs.length > 1) {
                species.NNs = species.NNs.slice(0, Math.ceil(species.NNs.length / 2));
            } 
            else {
                species.NNs = [];
            }

            let indexElites = Math.max(1, Math.floor(species.NNs.length * 1));
            let elites = species.NNs.slice(0, indexElites)

            res = [...res, ...elites];
        }

        if (superElite) {
            for (let i = 0; i < 10; i++) res.push(superElite);
        }
    ...
    crossoverNN(parent1, parent2) {
        ...
        // Crossover inputs
        for (const inp of dominant.inputs) {
            child.inputs.push(new Neuron(inp.activation, inp.id, inp.bias, inp.isInput, inp.isOutput));
        }
        // Ensure bias neuron is set
        child.inputs[child.inputs.length - 1].value = 1;

        // Crossover outputs
        for (const out of dominant.outputs) {
            child.outputs.push(new Neuron(out.activation, out.id, out.bias, out.isInput, out.isOutput));
        }
    ...
    newValue(){
        ...
        return getGaussianWeight()
    }

    mutateDelta(value){
        let delta = constrain(randomGaussian(0, this.randomConfig.mut_pow), -1, 1)
