class Nexus{
	...
	cargaAttack(){
		...
		let selected = selectActiveAttack.selected()

		if(selected == 'Thunder of Rays'){
			// TORMENTA DE RAYOS
		...
		else if(selected == 'Force Field Repulsion'){
			//REPULSION
			this.isAttacking = true
			fleet.update()
		}

		else if(selected == 'Mega Missile Launcher'){
			//MISSILE LAUNCHER
			ml = new MisileLauncher(createVector(mouseX, mouseY))
		}

		else if(selected == 'Black Hole X-22'){
			bh = new BlackHole(createVector(mouseX, mouseY))
		}
