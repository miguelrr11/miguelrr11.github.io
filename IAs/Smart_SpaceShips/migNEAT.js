...
class NN{
    constructor(n_in, n_out, func_hidden, func_out, population, softmax = true, x = 0, y = 0, w = width, h = height){
        ...
        this.orderedNetwork = this.getOrderedNeurons()
        this.neuronMap = new Map([...this.inputs, ...this.outputs, ...this.hidden].map(neuron => [neuron.id, neuron]))
    }
    ...
    update() {
        ...
        let orderedList = this.getOrderedNeurons()
        let neuronMap = this.neuronMap
    ...
    show(){
        ...
        for(let p of this.particles){ 
            ...
            p.show(neuron, (neuron.isInput || neuron.isOutput) && (neuron.id != this.inputs.length-1), out)
        }
        noFill()
        stroke(150)
        strokeWeight(1)
...
class Population{
    ...
    */
    evolvePopulation(simple = false){
        ...
        if(!simple) for(let nn of mutatedNNs){
            ...
            nn.neuronMap = new Map([...nn.inputs, ...nn.outputs, ...nn.hidden].map(neuron => [neuron.id, neuron]))
        }
        else{
    ...
    */
    ...
    crossoverNN(parent1, parent2) {
        ...
        child.inputs[child.inputs.length - 1].value = this.biasEnabled ? 1 : 0
