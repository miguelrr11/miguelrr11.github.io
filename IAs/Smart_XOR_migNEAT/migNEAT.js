...
class Population{
    ...
    */
    evolvePopulation(){
        ...
        //seleccion NNs
        let [toBeMutated, elites] = this.selectNNs()
        let mutatedNNs = []
        ...
        //crossover
        while(mutatedNNs.length < this.size - elites.length){
    ...
    selectNNs() {
        ...
        for (let [idx, species] of this.species.entries()) {
            ...
            if (species.NNs.length > 1) {
                species.NNs = species.NNs.slice(0, Math.ceil(species.NNs.length * 0.5));
            } 
            else {
                species.NNs = [];
            }

            const indexElites = Math.max(1, Math.floor(species.NNs.length * 0.2)); 
            const elitesAux = species.NNs.slice(0, indexElites);
            const toBeMutatedAux = species.NNs.slice();

            elites = [...elites, ...elitesAux];
            toBeMutated = [...toBeMutated, ...toBeMutatedAux];
        }
