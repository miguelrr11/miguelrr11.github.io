...
class Connector{
    ...
    getAnglesOutgoing(applyActiveness = false){
        ...
        let angles = []
        let outgoingSegs = this.outgoingSegmentIDs.map(id => this.road.findIntersecSeg(id))
    ...
    hover(x, y){
        ...
        return distt(x, y, this.pos.x, this.pos.y) <= CONN_RAD
    }
    ...
    getOutgoingActiveIntersegs(){
        ...
        let outgoingSegs = []
        let segs = this.outgoingSegmentIDs.map(id => this.road.findIntersecSeg(id))
        for(let seg of segs){
