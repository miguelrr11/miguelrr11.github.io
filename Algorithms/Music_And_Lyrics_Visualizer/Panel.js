...
class Panel{
	constructor(properties = {}) {
	    ...
	    if(this.lightCol[3]) this.transCol = this.lightCol.slice()
	    else this.transCol = [...this.lightCol, 90];
...
function getPixelLength(word, size, font){
	push()
	textSize(size)
...
function wrapText(text, maxWidth = this.w, textSize = text_SizeMIGUI, font = text_FontMIGUI) {
  ...
  words.forEach((word) => {
    ...
    if (getPixelLength(lineWithWord, textSize, font) <= maxWidth) {
