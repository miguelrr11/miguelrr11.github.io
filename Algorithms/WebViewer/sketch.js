...
function showTextBox(tb) {
    push();
    tb.counter = lerp(tb.counter, 100, 0.1)
    if(tb.counter > 100) {
        tb.counter = 100
    }
    let w = tb.w * (tb.counter * 0.01)
    let tbPos = tb.particle.pos.copy()
    tbPos.x -= (WIDTH_TB / 2) * (tb.counter * 0.01)
    tbPos.y -= offsetTB
    let innerW = w - offsetTB * 8;
    textSize(tb.counter * .1)
    let textH = getWrappedTextHeight(tb.text, innerW);
    tb.h = (textH + offsetTB * 12) * (tb.counter * 0.01)


    fill(curCol.background);
    noStroke();
    rect(tbPos.x, tbPos.y, w, tb.h, radiusTextBox);

    noFill();
    stroke(curCol.topographic);
    strokeWeight(1.5);
