...
class NN{
    ...
    show(){
        ...
        for(let p of this.particles){ 
            ...
            p.show(neuron.id, round(neuron.bias, 1), out, (neuron.isInput || neuron.isOutput) && (neuron.id != this.inputs.length-1))
        }
        noFill()
        stroke(150)
        strokeWeight(1)
...
class Population{
    ...
    */
    ...
    assignSpecies(compatibilityThreshold = 1.5) {
        //this.species = new Map();
    ...
    selectNNs() {
        ...
        for (let [idx, species] of this.species.entries()) {
            ...
            if (species.NNs.length > 1) {
                species.NNs = species.NNs.slice(0, Math.max(1, Math.ceil(species.NNs.length * 0.5)));
            }
...
function getSmallBias(){
    return constrain(randomGaussian(0, .4), -2, 2);  //[-1, 1]
}
